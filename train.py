...
def main():
    ...
    if args.action == 'train':
        ...
        #test_gdata=ImageDataGenerator(preprocessing_function=preprocess_input)
        #test_generate=test_gdata.flow_from_directory(args.valpath,target_size=(224, 224),batch_size=args.batchsize)
        #test_names=test_generate.filenames
        #print(test_names)
        #nb_test_samples=test_generate.samples
        ...
        for i,m_c in zip(range(len(imagenames)),m_class):
            ...
            print('[squeezenet_demo] %s: %s (%.2f)' % (str(imagenames[i]), str(_cls), max_prob))
        print(accpreds)
